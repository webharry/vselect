...
const semverify = version => {
  ...
  if (isString && _semver().default.valid(version)) {
    return version;
  }

  (0, _invariant().default)(typeof version === "number" || isString && versionRegExp.test(version), `'${version}' is not a valid version`);
